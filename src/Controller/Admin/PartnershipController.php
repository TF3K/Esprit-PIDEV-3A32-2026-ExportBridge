...
class PartnershipController extends AbstractController
{
    ...
    public function add(
        ...
        if ($request->isMethod('POST')) {
            ...
            $documentUrl     = trim($request->request->get('document_url') ?? '');

            $old = compact('companyId', 'type', 'status', 'establishedDate', 'terminatedDate', 'notes', 'documentUrl');
            ...
            $this->validateUrl($documentUrl, false);
            if ($this->hasValidationErrors()) {
                $errors['document_url'] = $this->getFirstValidationError();
                $this->clearValidationErrors();
            }
            ...
            if (empty($errors)) {
                $this->applyToEntity($partnership, $type, $status, $notes,
                    $establishedDate, $terminatedDate, $companyId, $companyRepo, $documentUrl);
                ...
                if ($partnership->getCompany()) {
                    $notifier->notifyCompany($partnership->getCompany(), 'partnership', 'Partnership created', 'A new partnership has been created with status ' . $partnership->getStatus() . '.', false);
                }
    ...
    public function edit(
        ...
        if ($request->isMethod('POST')) {
            ...
            if (empty($errors)) {
                ...
                if ($partnership->getCompany()) {
                    $notifier->notifyCompany($partnership->getCompany(), 'partnership', 'Partnership updated', 'Partnership status is now ' . $partnership->getStatus() . '.', false);
                }
    ...
    private function applyToEntity(
        ...
        $partnership->setDocumentUrl($documentUrl ?: null);
    }
}