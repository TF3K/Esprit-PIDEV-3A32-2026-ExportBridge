...
class ProductController extends AbstractController
{
    ...
    public function index(ProductRepository $repo, CacheInterface $cache): Response
    {
        // Cache products list for 2 minutes to improve response time
        $products = $cache->get('products_list', function (ItemInterface $item) use ($repo) {
            $item->expiresAfter(120); // 2 minutes
            return $repo->findAllWithRelations();
        });

        // Cache barcodes for 5 minutes (barcodes don't change frequently)
        $barcodeDataUris = $cache->get('products_barcodes', function (ItemInterface $item) use ($products) {
            $item->expiresAfter(300); // 5 minutes
            return array_map(
                fn($product) => $this->buildBarcodeDataUri($product),
                $products
            );
        });
