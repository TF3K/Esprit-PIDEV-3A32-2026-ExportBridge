...
public function create(Request $request, ManagerRegistry $doctrine): Response
{
    if ($request->isMethod('POST')) {
        ...
        // 1. Validation CSRF
        if (!$this->isCsrfTokenValid('market_create', $request->request->get('_token'))) {
        ...
        // 3. Validations de présence (Champs obligatoires)
        ...
        // 4. Validation du format (Lettres uniquement pour le titre)
        ...
        try {
            ...
            // Optionnel : Rediriger vers la liste des marchés après succès
...
#[Route('/market/list', name: 'app_market_list')]
    ...
    public function delete($id, ManagerRegistry $manager, MarketRepository $repo)
    {
...
{
    // Le Param Converter (Market $market) remplace le find($id) et le throw 404

    if ($request->isMethod('POST')) {
        // 1. Validation CSRF (Identifiant unique : market_update_ + ID)
        if (!$this->isCsrfTokenValid('market_update_' . $market->getId(), $request->request->get('_token'))) {
            throw $this->createAccessDeniedException('Invalid CSRF token');
        }
        ...
        // 4. Validation Format Titre (Lettres uniquement)
        if (!preg_match("/^[a-zA-ZÀ-ÿ\s]{3,50}$/", $name)) {
            $this->addFlash('error', 'Le titre du marché doit contenir uniquement des lettres (entre 3 et 50 caractères).');
            return $this->redirectToRoute('app_market_update', ['id' => $market->getId()]);
        }
        ...
        try {
            $em->flush(); // Pas besoin de persist() pour une mise à jour
            $this->addFlash('success', 'Le marché "' . $name . '" a été modifié avec succès !');
            
            // Rediriger vers la liste après succès (adaptez le nom de la route si nécessaire)
            return $this->redirectToRoute('app_admin_markets'); 
        } catch (\Exception $e) {
