...
class PartnershipController extends AbstractController
{
    ...
    public function add(
        ...
        if ($request->isMethod('POST')) {
            ...
            $this->validateRequired($companyId, 'Company', true);
            $this->validateSelection($type, ['Export', 'Import', 'Distribution', 'Joint Venture', 'Strategic Alliance'], 'Type', false);
            $this->validateSelection($status, ['active', 'pending', 'terminated', 'suspended'], 'Status', true);
            $this->validateRequired($notes, 'Notes', true, 10);
            $this->validateDate($establishedDate, 'Established date', false);
            $this->validateDate($terminatedDate, 'Terminated date', false);

            // Validate date range if both dates provided
            if ($establishedDate && $terminatedDate) {
