...
class ProductController extends AbstractController
{
    ...
    public function add(
        ...
        CompanieRepository $companieRepo,
        ProductCategorieRepository $categoryRepo
    ): Response {
        $product = new Product();

        if ($request->isMethod('POST')) {
            $this->handleForm($product, $request, $companieRepo, $categoryRepo);
        ...
        return $this->render('admin/products/form.html.twig', [
            'product'    => $product,
            'companies'  => $companieRepo->findAll(),
    ...
    private function handleForm(
        ...
        $product->setCompanie($companyId ? $companieRepo->find($companyId) : null);

        $categoryId = $request->request->get('category_id');
        $product->setProductCategorie($categoryId ? $categoryRepo->find($categoryId) : null);
    }
}